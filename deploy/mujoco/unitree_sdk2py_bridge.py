...
class VerticalPerturbation:
    """Constant vertical force on robot torso."""

    def __init__(self, force_magnitude=70.0):
        self.force_magnitude = force_magnitude  # Newtons
        self.enable = False
        self.direction = -1  # -1 = downward, +1 = upward

    def Advance(self, dt):
        """Returns [Fx, Fy, Fz] force to apply this step."""
        if not self.enable:
            return np.zeros(3)
        return np.array([0.0, 0.0, self.direction * self.force_magnitude])

    def MujocoKeyCallback(self, key):
        glfw = mujoco.glfw.glfw
        if key == glfw.KEY_SPACE:
            self.enable = not self.enable
            direction_str = "DOWN" if self.direction < 0 else "UP"
            print(f"[Z-Perturbation] {'ON' if self.enable else 'OFF'} (force={self.force_magnitude}N, {direction_str})")
        if key == glfw.KEY_LEFT_SHIFT or key == glfw.KEY_RIGHT_SHIFT:
            self.direction *= -1
            direction_str = "DOWN" if self.direction < 0 else "UP"
            print(f"[Z-Perturbation] direction = {direction_str}")
        if key == glfw.KEY_MINUS:
            self.force_magnitude = max(0, self.force_magnitude - 10)
            print(f"[Z-Perturbation] force = {self.force_magnitude}N")
        if key == glfw.KEY_EQUAL:  # "+" key
            self.force_magnitude += 10
            print(f"[Z-Perturbation] force = {self.force_magnitude}N")


class DirectionalPerturbation:
    """Arrow-key controlled XY force on robot torso.

    Controls:
        UP    -- push +X (forward)
        DOWN  -- push -X (backward)
        RIGHT -- push +Y (right)
        LEFT  -- push -Y (left)
        , (<) -- decrease magnitude by 10 N
        . (>) -- increase magnitude by 10 N
    """

    def __init__(self, force_magnitude=30.0, frequency=0.5):
        self.force_magnitude = force_magnitude
        self.frequency = frequency
        self._active = np.zeros(2)  # [x, y] direction: -1, 0, or +1
        self._elapsed = 0.0

    def Advance(self, dt):
        """Returns [Fx, Fy, Fz] force to apply this step."""
        if self._active[0] == 0 and self._active[1] == 0:
            self._elapsed = 0.0
            return np.zeros(3)
        self._elapsed += dt
        scale = self.force_magnitude * np.sin(2.0 * np.pi * self.frequency * self._elapsed)
        return np.array([self._active[0] * scale, self._active[1] * scale, 0.0])

    def MujocoKeyCallback(self, key):
        glfw = mujoco.glfw.glfw
        if key == glfw.KEY_UP:
            self._active[0] = 0.0 if self._active[0] == 1.0 else 1.0
            state = "ON +X" if self._active[0] == 1.0 else "OFF X"
            print(f"[XY-Arrow] {state} (force={self.force_magnitude}N)")
        elif key == glfw.KEY_DOWN:
            self._active[0] = 0.0 if self._active[0] == -1.0 else -1.0
            state = "ON -X" if self._active[0] == -1.0 else "OFF X"
            print(f"[XY-Arrow] {state} (force={self.force_magnitude}N)")
        elif key == glfw.KEY_RIGHT:
            self._active[1] = 0.0 if self._active[1] == 1.0 else 1.0
            state = "ON +Y" if self._active[1] == 1.0 else "OFF Y"
            print(f"[XY-Arrow] {state} (force={self.force_magnitude}N)")
            ...
            self.force_magnitude = max(0, self.force_magnitude - 10)
            print(f"[XY-Arrow] force = {self.force_magnitude}N")
        elif key == glfw.KEY_PERIOD:  # ">" key
            self.force_magnitude += 10
            print(f"[XY-Arrow] force = {self.force_magnitude}N")


class LateralPerturbation:
    """Random lateral (XY) impulse pushes on robot torso."""

    def __init__(self, force_magnitude=70.0, pulse_duration=0.3, interval=3.0):
        self.force_magnitude = force_magnitude  # Newtons
        self.pulse_duration = pulse_duration    # seconds
        self.interval = interval                # seconds between pushes
        self.enable = False
        self.elapsed = 0.0
    ...
    def _new_push(self):
        """Sample a random XY direction."""
        angle = np.random.uniform(0, 2 * np.pi)
        self._direction = np.array([np.cos(angle), np.sin(angle)])
    ...
    def Advance(self, dt):
        ...
        if not self.enable:
            return np.zeros(3)
        self.elapsed += dt
        cycle_pos = self.elapsed % self.interval
        if cycle_pos < dt:
            self._new_push()
        if cycle_pos < self.pulse_duration:
            fxy = self._direction * self.force_magnitude
            return np.array([fxy[0], fxy[1], 0.0])
        return np.zeros(3)
    ...
    def MujocoKeyCallback(self, key):
        ...
        if key == glfw.KEY_P:
            self.enable = not self.enable
            print(f"[XY-Perturbation] {'ON' if self.enable else 'OFF'} (force={self.force_magnitude}N)")
        if key == glfw.KEY_LEFT_BRACKET:  # "[" key
            self.force_magnitude = max(0, self.force_magnitude - 10)
            print(f"[XY-Perturbation] force = {self.force_magnitude}N")
        if key == glfw.KEY_RIGHT_BRACKET:  # "]" key
            self.force_magnitude += 10
            print(f"[XY-Perturbation] force = {self.force_magnitude}N")
