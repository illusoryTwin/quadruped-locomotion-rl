...
class CommandServer(threading.Thread):
    ...
    def __init__(self, deployer, port: int = 7777):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as srv:
            ...
            while True:
                try:
                    ...
                    print(f"[CommandServer] Error: {e}")

    def _handle(self, raw: str) -> str:
        try:
            msg = json.loads(raw)
            if "stiffness_commands" not in msg:
                return json.dumps({"status": "error", "message": "missing stiffness_commands key"})
...
Examples:
  ...
  python deploy/deploy.py \\
    ...
    parser.add_argument("--stiffness-port", type=int, default=7777,
                        help="TCP port for runtime stiffness injection (0 to disable, default 7777)")
    ...
    deployer = Go2PolicyDeployer(
        ...
        stiffness_port=args.stiffness_port,
    )
